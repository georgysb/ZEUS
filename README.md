...
**ZEUS** is a custom modular standalone 8-bit computer kit for boys 40+ and computer science students. It was designed to be robust and handy for **Z80** assembly language learning and "from the scratch" code developing.
...
- ZEUS SIO module is compatible with any Z80 family SIO IC (0,1 or 2)
- ZEUS MEM128k module has WR-WRE jumper which provides (if shorted) opportunity to write data to EEPROM right in the module.
  But be advised that AT28C256-15PU is a bit slow for 10MHz system clock, so if you don't want to corrupt EEPROM data you should switch system clock to 7.3728MHz (by removing 10MHz generator from the socket on ZEUS CPU module then closing CLK-UCLK jumper on ZEUS SIO module and reseting Zeus) before writing data to EEPROM. But if you use FM1808-70-P (FRAM) instead of AT28C256-15PU (EEPROM), you need no hassle with system clock change.
