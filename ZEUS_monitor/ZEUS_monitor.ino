...
0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, // hex table 0x8004
0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, // hex table 0x8014

0x64, 0x75, 0x6D, 0x70, 0x20, // 0x8024 "dump "
0x72, 0x75, 0x6E, 0x20,       // 0x8029 "run "
0x63, 0x6C, 0x72,             // 0x802D "clr"
0x6A, 0x75, 0x6D, 0x70, 0x20, // 0x8030 "jump "
0x6C, 0x6F, 0x61, 0x64, 0x20, // 0x8035 "load "
0x65, 0x78, 0x65, 0x63, 0x20, // 0x803A "exeс "
0x73, 0x65, 0x6E, 0x64, 0x20, // 0x803F "send "
0x73, 0x79, 0x73, 0x72, 0x20, // 0x8044 "sysr "
0x63, 0x72, 0x63, 0x37, 0x20, // 0x8049 "crc7 "
0x73, 0x64, 0x69, 0x20,       // 0x804E "sdi "
0xFF};
...
0xFB, 0xED, 0x4D, //>SIO pass 0x9300(+0x0003)
//---------------
0x0E, 0xFF, 0x0D, 0x20, 0xFD, 0x05, 0x20, 0xF8, 0xC9, //>Pause xx*4096 ticks (input B) 0x9303(+0x0009)
//---------------
0x7A, 0xD3, 0x11, 0x3E, 0x20, 0xD3, 0x10, 0x3E, 0x61, 0xD3, 0x10, 0xC9, // TFT_wr_com (input D) 0x930C(+0x000C)
0x7A, 0xD3, 0x11, 0x3E, 0xA0, 0xD3, 0x10, 0x3E, 0xE1, 0xD3, 0x10, 0xC9, // TFT_wr_data (input D) 0x9318(+0x000C)
0x7A, 0xD3, 0x11, 0x3E, 0xC0, 0xD3, 0x10, 0x3E, 0xE1, 0xD3, 0x10, 0xC9, // TFT_rd_data (input D) 0x9324(+0x000C)
//---------------
0xE5, 0xC5, 0xD5, //>TFT_char_cell_set (input D, E) 0x9330(+0x0059)
...
0xD1, 0xC1, 0xE1, 0xC9, 
//---------------
0xE5, 0xC5, 0xD5, //>TFT_char_fill (input A) 0x9389(+0x0050)
0x6F, 0x26, 0x00, 0x0E, 0x04, 0x37, 0x3F, 0xCB, 0x15, 0xCB, 0x14, 0x0D, 0x20, 0xF7,
0x16, 0x2C, 0xCD, 0x0C, 0x93, 
0xED, 0x4B, 0x00, 0xFE, 0x09, // LD BC, ($7F00) / ADD HL, BC
...
0xD1, 0xC1, 0xE1, 0xC9, 
//---------------
0x3A, 0x04, 0xFE, 0xFE, 0x00, 0xC8, 0x3D, 0x32, 0x04, 0xFE, //>TFT_ch_erase 0x93D9(+0x0027)
0xED, 0x5B, 0x0C, 0xFE, 0x3E, 0x00, 0xBA, 0x20, 0x06, 0xBB, 
0x28, 0x07, 0x1D, 0x16, 0x28, 0x15, 0xCD, 0x30, 0x93, 0x3E, 
0x20, 0xCD, 0x89, 0x93, 0xED, 0x53, 0x0C, 0xFE, 0xC9, 
//---------------
0xDB, 0x00, 0x2A, 0x06, 0xFE, 0x77, 0x23, 0x22, 0x06, 0xFE, 0xFB, 0xED, 0x4D, //>SIO read UART data (push ($FE06)) 0x9400(+0x000D)
//---------------
0xC5, 0xDB, 0x01, 0x4F, 0x21, 0x80, 0xFE, 0x46, //>SIO read PS/2 data (push $HL) 0x940D(+0x009D)
0x3E, 0xE0, 0xB8, 0x28, 0x88, 0xB9, 0x28, 0x85, //check E0 block
...
0x3E, 0x66, 0xB9, 0x20, 0x0E, 0x3E, 0xF0, 0xB8, 0x28, 0x1A, 0x3E, 0x08, 0xD3, 0x00, 0xCD, 0xD9, 0x93, 0x28, 0x0F, //check Backspace (call char_erase)
0x3E, 0x5A, 0xB9, 0x20, 0x0C, 0x3E, 0xF0, 0xB8, 0x20, 0x07, 0x3E, 0x0D, 0xD3, 0x00, 0xCD, 0x8B, 0x95, // check Enter release
...
0xC5, 0xE5, //>Hex_parse (input DE output A) 0x9500(+0x0042)
0x01, 0x10, 0x00, 0x21, 0x23, 0x80, 0x7A, 0xED, 0xB9, 0x79, 0x21, 0xFF, 0xFE, 0x28, 0x0D, 
0x01, 0x10, 0x00, 0x21, 0x13, 0x80, 0x7A, 0xED, 0xB9, 0x79, 0x21, 0xFF, 0xFE, 0xED, 0x6F, 
0x01, 0x10, 0x00, 0x21, 0x23, 0x80, 0x7B, 0xED, 0xB9, 0x79, 0x21, 0xFF, 0xFE, 0x28, 0x0D, 
0x01, 0x10, 0x00, 0x21, 0x13, 0x80, 0x7B, 0xED, 0xB9, 0x79, 0x21, 0xFF, 0xFE, 0xED, 0x6F, 
0x7E, 0xE1, 0xC1, 0xC9, 
//---------------
0xC5, 0xE5, 0x4F, //>Hex_print (input A) 0x9542(+0x002A)
0x21, 0xFF, 0xFE, 0x71, 0xAF, 0xED, 0x6F, 0xC6, 0x04, 0x6F, 0x26, 0x80, 0x7E, 0xCD, 0x89, 0x93, 
0x21, 0xFF, 0xFE, 0x71, 0xAF, 0xED, 0x67, 0xC6, 0x04, 0x6F, 0x26, 0x80, 0x14, 0xCD, 0x30, 0x93, 0x7E, 0xCD, 0x89, 0x93, 
0xE1, 0xC1, 0xC9, 
//---------------
0xC5, 0xE5, //>syntax error 0x956C(+0x001F)
...
0xE1, 0xC1, 0xC9, 
//---------------
0xC5, 0xE5, 0x3A, 0x05, 0xFE, 0x67, //>Cmdline_parser  0x958B(+0x00CE)
0x2E, 0x00, 0x11, 0x2D, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x10, 0x1C, 0xE5, 0x7D, 0x21, 0x04, 0xFE, 0xBE, 0xE1, 0x20, 0xF1, 0xCD, 0x86, 0x97, 0xE1, 0xC1, 0xC9, // "clr" check(26)
0x2E, 0x00, 0x11, 0x24, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x00, 0x97, 0xE1, 0xC1, 0xC9, // "dump " check(21)
0x2E, 0x00, 0x11, 0x30, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x6C, 0x97, 0xE1, 0xC1, 0xC9, // "jump " check
0x2E, 0x00, 0x11, 0x3A, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x04, 0x98, 0xE1, 0xC1, 0xC9, // "exec " check
0x2E, 0x00, 0x11, 0x35, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x39, 0x98, 0xE1, 0xC1, 0xC9, // "load " check
0x2E, 0x00, 0x11, 0x3F, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x74, 0x98, 0xE1, 0xC1, 0xC9, // "send " check
0x2E, 0x00, 0x11, 0x44, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0xB9, 0x98, 0xE1, 0xC1, 0xC9, // "sysr " check
0x2E, 0x00, 0x11, 0x49, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0xD5, 0x98, 0xE1, 0xC1, 0xC9, // "crc7 " check
0x2E, 0x00, 0x11, 0x4E, 0x80, 0x2C, 0x1A, 0xBE, 0x20, 0x0B, 0x1C, 0xFE, 0x20, 0x20, 0xF6, 0xCD, 0x10, 0x99, 0xE1, 0xC1, 0xC9, // "sdi " check
0xCD, 0x6C, 0x95, // go syntax error
0xE1, 0xC1, 0xC9, 
0xFF};
...
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x05, // "dump " command 0x9700(+0x006C)
...
0xCD, 0x30, 0x93, 0x78, 0xCD, 0x42, 0x95, 0x14, 0xCD, 0x30, 0x93, 0x79, 0xCD, 0x42, 0x95, 0x14, 0xCD, 0x30, 0x93, 0x3E, 0x3A, 0xCD, 0x89, 0x93, 0x14, 
0xCD, 0x30, 0x93, 0x3E, 0x20, 0xCD, 0x89, 0x93, 0x14, 0xCD, 0x30, 0x93, 0x0A, 0xCD, 0x42, 0x95, 
0x03, 0x14, 0xD9, 0x0D, 0x28, 0xBC, 0xD9, 0x18, 0xE7, 
0x21, 0x80, 0x00, 0x22, 0x08, 0xFE, //color R
0xD9, 0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x05, //>"jump " command 0x976C(+0x001A)
0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 0x47, 
0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 0x4F, 
0xC5, 0xE1, 0xE9, 0xC9, 
//---------------
0x16, 0x36, 0xCD, 0x0C, 0x93, 	//>"clr" command 0x9786(+0x007E)
...
0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x04, 0x01, 0x00, 0x80, //>"exeс " command 0x9804(+0x0035)
...
0x3E, 0xC9, 0x0C, 0x02, 0xCD, 0x00, 0x80, 0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x05, //>"load " command 0x9839(+0x003B)
0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 0x47, 
0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 0x4F, 
...
0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x05, //>"send " command 0x9874(+0x0045)
...
0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x05, //>"sysr " command 0x98B9(+0x001C)
0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 
...
0xC9, 
//---------------
0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x04, 0x01, 0x00, 0x01, //>"crc7 " command 0x98D5(+0x003B)
...
0xCD, 0x00, 0x9A, //call crc7_add
0x1E, 0x01, 0x16, 0x00, 0xCD, 0x30, 0x93, //set print pos, 
0x7E, 0xCD, 0x42, 0x95, // print (HL)
0xC9, 
//---------------
0xDD, 0x21, 0xFE, 0xFE, 0x3A, 0x05, 0xFE, 0x67, 0x2E, 0x03, //>"sdi " command (L bytes at $0100) 0x9910(+0x0039)
0x2C, 0x2C, 0x56, 0x2C, 0x5E, 0xCD, 0x00, 0x95, 0xDD, 0x77, 0x00, 
0x4D, 0x21, 0x00, 0x01, 0x3E, 0x00, 0xD3, 0x01, 0xCD, 0x3D, 0x9A, 0x23, 0x0B, 0x28, 0x02, 0x18, 0xF7, 
0xDD, 0x4E, 0x00, 0x21, 0x00, 0xFC, 0xCD, 0xBF, 0x9A, 0x23, 0x0D, 0x20, 0xF9, 
0x3E, 0x01, 0xD3, 0x01, 
0xC9, 
...
//-------------------------------------------------------------------------------- SD routines
int offset_sd = 0x1A00;
const byte BLOCK_SD[] = {
0xC5, 0xE1, 0x7E, 0x26, 0xFC, 0x77, 0xAF, 0xBD, 0x28, 0x04, 0x2D, 0x60, 0x18, 0xF4, //crc7_add [input BC last byte pointer] 0x9A00(+0x003D)
...
//---------------
0xAF, 
0xC8, 0x7E, 0xC8, 0xCF, 0x20, 0x02, 0xC8, 0x8F, 0xC8, 0xDF, 0xD3, 0x01, 0xC8, 0x9F, 0xD3, 0x01, //SD wr byte [input HL] 0x9A3D(+0x0082)
...
0xC9, 
//---------------
0xC5, 0x0E, 0x08, //SD rd byte [output (HL)] 0x9ABF(+0x0018)
...
0x3E, 0x81, 0xED, 0x47, //set int table to 0x8100
//---------------------------
//---------------------------------------------------- pointers & 2BYTE values
...
0x16, 0x39, 0xCD, 0x0C, 0x93, // com (0x39) Idle mode ON
0x16, 0x11, 0xCD, 0x0C, 0x93, // com (0x11) Exit sleep

0x06, 0xFF, 0xCD, 0x03, 0x93, // pause FF*4096
...
//0x3E, 0x41, 0xCD, 0x89, 0x93, // print "A"
//0x14, 0xCD, 0x30, 0x93, //INC D + call TFT_char_cell_set
0x3E, 0x3E, 0xCD, 0x89, 0x93, // print ">"
0x14, //INC D
0xED, 0x53, 0x0C, 0xFE, //LD ($FE08), DE

//0x11, 0x01, 0x00, //LD DE, 0x00, 0x01
//0xCD, 0x30, 0x93, //call TFT_char_cell_set
//0x3E, 0x4F, 0xCD, 0x89, 0x93, // print "O"
//0x14, 0xCD, 0x30, 0x93, //INC D + call TFT_char_cell_set
//0x3E, 0x4B, // print "K"

0x3E, 0xFE, 0xD3, 0xF0, //write 0xFE to sys reg (switch to bank E)
0x21, 0x00, 0xC3, //LD HL with new jump address
0x22, 0x00, 0x00, // write HL to $0000
0x21, 0x00, 0xA0, //LD HL with new jump address
0x22, 0x02, 0x00, // write HL to $0002
...
//0x3E, 0x4F, 0xD3, 0x00, //"O"
//0x3E, 0x4B, 0xD3, 0x00, //"K"
//0x3E, 0x0D, 0xD3, 0x00, //"cr"
...
0x21, 0x00, 0x00, 0x11, 0x00, 0x80, 0x01, 0x00, 0x30, 0xED, 0xB0, //copy block 0x0000 -> 0x8000
0x21, 0x00, 0xA0, //LD HL with new jump address
0x22, 0x02, 0x80, // write HL to $8002
0xC3, 0x00, 0x80, //jump high mem
0xFF}; //SIO interrupt table
...
//    for(int j=0; j<0x100; j++){writeEEPROM(0x1900 + j, 0xFF); byteswrit++;}
...
//    for(int j=0; j<blocksize_primer; j++){writeEEPROM(offset_primer + j, BLOCK_PRIMER[j]); byteswrit++;}
    digitalWrite(ADDR[15], 1);
    Serial.print(byteswrit);
    Serial.println(" bytes written");
//    Serial.println(blocksize_0);
  }
  if(Serial.available()){
    addr_in = int(Serial.parseInt());
    byte data1 = readEEPROM(addr_in);
    sprintf(output, "%04x %02x", addr_in, data1);
